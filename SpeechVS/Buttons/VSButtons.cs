...
public class VSGenericButton : VSButton
{
    ...
    {
        SpeechVS.Logger.LogInfo("Populating button: " + name);
        if (knownParent == null)
        {
            SpeechVS.Logger.LogError(this.name + " had null PARENT?");
        }
        this.name = name;
        Transform buttonObject = knownParent.Find(path);
        if (buttonObject == null)
        {
            SpeechVS.Logger.LogError(this.name + " had null buttonObject at " + path);
        }
        Transform collider = knownParent.Find(path + colliderPath);
        Transform highlight = knownParent.Find(path + highlightPath);
        if (collider == null)
        {
            SpeechVS.Logger.LogError(this.name + " had null collider at " + path + colliderPath);
        }
        ...
        {
            SpeechVS.Logger.LogError(this.name + " had null buttonFsm");
        }
        if (highlight == null)
        {
            SpeechVS.Logger.LogError(this.name + " had null highlight at " + path + highlightPath);
        }
        this.components.buttonObject = buttonObject.gameObject;
        this.components.highlight = highlight.gameObject;
        SpeechVS.Logger.LogInfo("Verified button: " + this.name);
    }
...
public class VSChoiceButton : VSGenericButton
{
    ...
    public VSChoiceButton(Transform knownParent, string name, string path, string colliderPath, string highlightPath, ButtonType buttonType)
        : base(knownParent, name, path, colliderPath, highlightPath)
    {
        ...
        try
        {
            ...
            if (buttonObject == null)
            {
                SpeechVS.Logger.LogWarning($"[VSChoiceButton] Could not find button object at path: {path}");
                return;
            }

            Transform textTransform = buttonObject.Find("DoneBG/DoneText/Text (TMP)");
            if (textTransform == null)
            {
                textTransform = buttonObject.Find("DoneBG/DoneText/ButtonText");
                if (textTransform == null)
                {
                    textTransform = buttonObject.Find("DoneBG/DoneText/DisobeyText");
                    if (textTransform == null)
                    {
                        SpeechVS.Logger.LogWarning($"[VSChoiceButton] Could not find Text (TMP) object under: {path}");
                        return;
                    }
                }
            }

            TMP_Text tmp_Text = textTransform.GetComponent<TMP_Text>();
            if (tmp_Text == null)
            {
                SpeechVS.Logger.LogWarning($"[VSChoiceButton] TMP_Text component missing on object: {textTransform.name}");
                return;
            }

            this.triggerWord = tmp_Text.text;
            if (this.triggerWord == null)
            {
                SpeechVS.Logger.LogWarning($"[VSChoiceButton] TMP_Text text null: {name}");
                this.triggerWord = "notabletobeused";
            }
            else
            {
                SpeechVS.Logger.LogInfo("Button Word: " + this.triggerWord);
            }
        }
        catch (Exception ex)
        {
            SpeechVS.Logger.LogError($"[VSChoiceButton] Exception during initialization: {ex.Message}");
        }
    }
...
public class VSFindomButton : VSButton
{
    ...
    public VSFindomButton(Transform knownParent, string name, string path)
    {
        ...
        if (buttonObject == null)
        {
            SpeechVS.Logger.LogError(this.name + " had null button object");
        }
        this.buttonObject = buttonObject.gameObject;
        this.button = buttonObject.GetComponent<Button>();
        if (this.button == null)
        {
            SpeechVS.Logger.LogError(this.name + " had null button");
        }
        highlight = this.buttonObject.GetComponent<Image>();
        if (this.highlight == null)
        {
            SpeechVS.Logger.LogError(this.name + " had null image");
        }

        SpeechVS.Logger.LogInfo("Verified findom button: " + this.name);
    }
...
public class VSStatusCancelButton : VSGenericButton
{
    ...
    public VSStatusCancelButton(Transform knownParent, string name, string path) : base(knownParent, name, path, colliderPath, highlightPath)
    {
        ...
        if (tf == null)
        {
            SpeechVS.Logger.LogError(this.name + " had null image transform at " + highlightPath);
        }
